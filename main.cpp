...
void drawSun()
{
    ...
    {
        float x1 = 32*cosf(a*D2R);
        float y1 = 32*sinf(a*D2R);

        float x2 = 48*cosf(a*D2R);
        float y2 = 48*sinf(a*D2R);
...
void drawInfoPanel()
{
    ...
    drawStr12(bx + 10, ly, "Diameter :");
    glColor3f(1.00f, 1.00f, 1.0f);
    drawStr12(bx + 108, ly, p->diameter);

    ly -= ls;
    glColor3f(0.55f, 0.80f, 1.0f);
    drawStr12(bx + 10, ly, "Distance :");
    glColor3f(1.00f, 1.00f, 1.0f);
    drawStr12(bx + 108, ly, p->distSun);

    ly -= ls;
    glColor3f(0.55f, 0.80f, 1.0f);
    drawStr12(bx + 10, ly, "Orbit    :");
    glColor3f(1.00f, 1.00f, 1.0f);
    drawStr12(bx + 108, ly, p->orbPeriod);

    ly -= ls;
    glColor3f(1.00f, 0.88f, 0.35f);
    drawStr10(bx + 10, ly, p->funFact);
...
void display()
{
    ...
    glScalef(zoom, zoom, 1.0f);              //SCALING FUNCTION
    glRotatef(sceneAngle, 0, 0, 1);         // ROTATION FUNCTION
    glTranslatef(-camX, -camY, 0.0f);       // TRANSLATION FUNCTION
...
void update(int v)
{
    if (!paused)
    {
        for (int i = 0; i < 6; i++)
        {
            ...
            if (planets[i].selfAngle >= 360.0f)
                planets[i].selfAngle -= 360.0f;
        }
        moonAngle += 13.5f;
        if (moonAngle >= 360.0f)
            moonAngle -= 360.0f;
